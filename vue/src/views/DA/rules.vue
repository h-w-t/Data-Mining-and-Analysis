<template>
  <div>
    <el-row style=" margin-bottom: 20px;width: 100%; height: 30%; display: flex; justify-content: center; align-items: center;">
      <el-card style="width: 40%; height: 40%;" class="box-card">
        ...
        <el-input placeholder="请输入置信度" v-model="input1" clearable style="margin-bottom: 10px;"></el-input>
        <el-input placeholder="请输入支持度" v-model="input2" clearable style="margin-bottom: 10px;"></el-input>
      </el-card>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    generateHeatmap() {
      const url = 'http://localhost:8000/apis/rules/result/';
      axios.get(url)
          .then(response => {
    ...
    analyzeData() {
      const url_data = 'http://localhost:8000/apis/rules/result/'; // 后端接口的URL，请根据实际情况修改
      axios.get(url_data)
          .then((response) => {
